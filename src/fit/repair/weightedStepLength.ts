...
export function weightedStepLengthCandidate(activity: NormalizedActivity): RepairCandidate {
  ...
  for (let index = 1; index < activity.records.length; index += 1) {
    ...
    const distanceDelta =
      current.distanceM != null && previous.distanceM != null
    ...
    if (
      ...
      distanceDelta >= 20 ||
      stepLength == null ||
      !isPlausibleStepLength(stepLength)
    ) {
      rejected += 1;
      continue;
    }
  ...
  if (stepLengthM == null || stepEstimate.totalSteps <= 0)
    throw new Error('Not enough high-quality samples for weighted step-length repair.');
  ...
  return {
    ...
    description:
      'Weights plausible step lengths by segment steps and quality, with a cap to limit damaged segments.',
    distanceM,
