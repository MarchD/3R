...
export function applyRepairPatch(
  ...
  const patchByIndex = new Map(candidate.recordPatches.map((record) => [record.recordIndex, record]));
  const repairedActivity: RepairedActivity = {
    ...
    records: original.records.map((record) => {
      ...
      return change ? { ...record, distanceM: change.distanceM, speedMps: change.derivedSpeedMps, enhancedSpeedMps: change.derivedSpeedMps } : { ...record };
    }),
    developerFields: {
    ...
    repair: { algorithm: candidate.algorithm, createdAt, originalDistanceM: original.session?.totalDistanceM },
  };
