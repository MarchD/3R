...
import { confidenceFromSampleCount } from './confidence';
...
export function stepLengthSamples(
...
export function buildStepDistancePatches(
  ...
  for (let index = 1; index < activity.records.length; index += 1) {
    ...
    if (dt != null && dt > 0 && dt <= 3 && cadence != null)
      totalIntegratedSteps += (cadence * dt) / 60;
  ...
  return activity.records.map((record, index) => {
    ...
    const dt = index
      ? secondsBetween(record.timestamp, activity.records[index - 1].timestamp)
      : undefined;
...
export function medianStepLengthCandidate(activity: NormalizedActivity): RepairCandidate {
  ...
  if (stepLengthM == null || stepEstimate.totalSteps <= 0)
    throw new Error('Not enough valid cadence and step-length data for median repair.');
  ...
  return {
    ...
    confidence: confidenceFromSampleCount(samples.length),
