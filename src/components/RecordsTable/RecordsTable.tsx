...
const columnDefinitions = {
  ...
  longitude: {
    label: 'Longitude',
    value: (r: NormalizedRecord) => r.position?.longitude?.toFixed(6),
  },
  distance: { label: 'Distance m', value: (r: NormalizedRecord) => r.distanceM?.toFixed(2) },
  speed: {
    label: 'Speed m/s',
    value: (r: NormalizedRecord) => (r.enhancedSpeedMps ?? r.speedMps)?.toFixed(3),
  },
  ...
  altitude: {
    label: 'Altitude m',
    value: (r: NormalizedRecord) => (r.enhancedAltitudeM ?? r.altitudeM)?.toFixed(1),
  },
  heading: { label: 'Heading', value: (r: NormalizedRecord) => r.headingDeg?.toFixed(1) },
  developer: {
    label: 'Developer fields',
    value: (r: NormalizedRecord) =>
      Object.keys(r.developerFields).length ? JSON.stringify(r.developerFields) : '—',
  },
};
...
function RecordRow({
  ...
  records,
  columns,
}: RowComponentProps<{ records: NormalizedRecord[]; columns: Column[] }>) {
  return (
    <div
      className="recordRow"
      style={{ ...style, gridTemplateColumns: `repeat(${columns.length}, minmax(120px, 1fr))` }}
    >
      {columns.map((column) => (
        <span key={column}>{columnDefinitions[column].value(records[index]) ?? '—'}</span>
      ))}
    </div>
  );
}
  ...
  return (
    <section className="recordsPanel">
      <details className="columnPicker">
        <summary>
          <Columns3 size={16} /> {t('records.configure')}
        </summary>
        <div>
          {defaults.map((column) => (
            <label key={column} htmlFor={`record-column-${column}`}>
              <input
                id={`record-column-${column}`}
                type="checkbox"
      ...
      <div className="recordTableScroll">
        ...
        >
          {columns.map((column) => (
            <strong key={column}>{t(`records.${column}`)}</strong>
          ))}
        </div>
        <List
