...
export function ActivitySummary({ result }: { result: ParsedFitFile }) {
  ...
  const gpsRecords = normalized.records.filter(
    (record) => record.position?.latitude != null && record.position?.longitude != null,
  ).length;
  const paceSeconds =
    session?.totalDistanceM && session.totalElapsedTimeS
      ? session.totalElapsedTimeS / (session.totalDistanceM / 1000)
      : undefined;
  const metrics = [
    ...
    [
      t('summary.started'),
      session?.startTime
        ? new Date(session.startTime).toLocaleString(language === 'uk' ? 'uk-UA' : 'en-US')
        : '—',
    ],
    ...
    [
      t('summary.distance'),
      session?.totalDistanceM == null ? '—' : `${(session.totalDistanceM / 1000).toFixed(3)} km`,
    ],
    ...
    [
      ...
      session?.avgHeartRate == null
        ? '—'
        : `${session.avgHeartRate} ${t('summary.avg')} · ${session.maxHeartRate ?? '—'} ${t('summary.max')}`,
    ],
  ...
  const crcLabel = {
    true: t('summary.crcValid'),
    false: t('summary.crcMismatch'),
    unknown: t('summary.crcUnknown'),
  }[integrity.crcValid == null ? 'unknown' : String(integrity.crcValid)];
  return (
    <div className="summaryLayout">
      ...
      <section className={`integrity ${integrity.crcValid ? 'valid' : 'warning'}`}>
        ...
        <div>
          <strong>{crcLabel}</strong>
          <p>{integrity.complete ? t('summary.completeBody') : t('summary.reviewBody')}</p>
        </div>
      </section>
      {!!integrity.warnings.length && (
        <section className="noticeList">
          <h3>{t('summary.warnings')}</h3>
